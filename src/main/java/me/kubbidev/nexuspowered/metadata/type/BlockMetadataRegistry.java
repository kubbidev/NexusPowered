...
public interface BlockMetadataRegistry extends MetadataRegistry<BlockPosition> {
     ...
     */
    @NotNull MetadataMap provide(@NotNull Block block);

    /**
     * Gets a {@link MetadataMap} for the given block, if one already exists and has been cached in this registry.
     *
     * @param block the block
     * @return a metadata map, if present
     */
    @NotNull Optional<MetadataMap> get(@NotNull Block block);
     ...
     */
    @NotNull <K> Map<BlockPosition, K> getAllWithKey(@NotNull MetadataKey<K> key);
}