...
public interface PlayerMetadataRegistry extends MetadataRegistry<UUID> {
     ...
     */
    @NotNull MetadataMap provide(@NotNull Player player);

    /**
     * Gets a {@link MetadataMap} for the given player, if one already exists and has been cached in this registry.
     *
     * @param player the player
     * @return a metadata map, if present
     */
    @NotNull Optional<MetadataMap> get(@NotNull Player player);
     ...
     */
    @NotNull <K> Map<Player, K> getAllWithKey(@NotNull MetadataKey<K> key);
}