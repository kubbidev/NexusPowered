...
public class FireworkBuilder extends BaseItemBuilder<FireworkBuilder> {
    ...
    FireworkBuilder(ItemStack itemStack) {
        super(itemStack);
        if (itemStack.getType() != STAR && itemStack.getType() != ROCKET) {
            throw new InventoryException("FireworkBuilder requires the material to be a FIREWORK_STAR/FIREWORK_ROCKET!");
        }
    }
     ...
     * @throws IllegalArgumentException If any effect is null (may be thrown after changes have occurred)
     */
    public FireworkBuilder effect(List<FireworkEffect> effects) {
        if (effects.isEmpty()) {
            return this;
        }
